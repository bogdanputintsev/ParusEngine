...
namespace parus::vulkan
{
    ...
    void VkRenderPassFactory::build(const std::string& name, VulkanStorage& storage)
    {
		...
		depthAttachment.format = utils::findDepthFormat(storage);
		...
		ASSERT(vkCreateRenderPass(storage.logicalDevice, &renderPassInfo, nullptr, &storage.renderPass) == VK_SUCCESS,
			"Failed to create render pass.");

        utils::setDebugName(storage, storage.renderPass, VK_OBJECT_TYPE_RENDER_PASS, name.c_str());
    }
}
